...
previous_D_TL=[0,0,0,0,0,0,0,0,0];
previous_result=200;
...
% for index=1:Move_num
    ...
    while ishandle(h)
        ...
        result = max(20, min(double(result), 450));
        
        current=round(result);
    
        distance_dtl=norm(last_D_TL(end, :) - previous_D_TL)
    
        current = current +distance_dtl*1;
        result = max(20, min(double(current), 450));
        ...
        if abs(result - previous_result) <5
            if ~isnan(command_time)
                response_time = time_now;
                recorded_response_times = [recorded_response_times; (response_time - command_time)];
                ...
                [var(tendon7)],[var(tendon8)],[var(tendon9)],[tail(recorded_response_times,1)],'VariableNames', ...
                {'current','x','y','z','M1','M2','M3','vx','vy','vz','vM1','vM2', ...
                'vM3','vM4','vM5','vM6','vM7','vM8','vM9','response'})
                writetable(DataFrame,"dynamic_pid_multiple.csv","WriteMode","append")
                tendon1=[];
                tendon2=[];
                tendon3=[];
                tendon4=[];
