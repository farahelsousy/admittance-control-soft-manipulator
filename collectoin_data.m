...
%         end
    ...
    Deform_Motion(index,:)=Deform_arm;
    figure(2)
    hold on
    [~,SM_sec_Motion(:,:,:,index)]=Config_Arm_draw(Deform_Motion(index,:),L0_seg,N_ele,0);
    hold off
    axis equal
    axis([-400 400 -400 400 -100 500])
    text(SM_sec_Motion(1,4,end,index),SM_sec_Motion(2,4,end,index),SM_sec_Motion(3,4,end,index),['M',num2str(index)])
    xlabel('x')
    ylabel('y')
    box on
    grid on
    ...
    angles=rad2deg(tform2axang(SM_sec(:,:,2)));
    last_D_TL=tail(D_TL, 1);
    DataFrame =table([ip_curr] , ...
        [SM_sec(1:1,4:4,2)] ,[SM_sec(2:2,4:4,2)],[SM_sec(3:3,4:4,2)], ...
        [angles(1)],[angles(2)] ...
        ,[last_D_TL(1)],[last_D_TL(2)],[last_D_TL(3)], ...
        [last_D_TL(4)],[last_D_TL(5)],[last_D_TL(6)], ...
        [last_D_TL(7)],[last_D_TL(8)],[last_D_TL(9)] ...
        ,'VariableNames',{'current','x','y','z', ...
        'rotx','roty','M1','M2','M3','M4','M5','M6','M7','M8','M9'})
    writetable(DataFrame,"3_sections_same2.csv","WriteMode","append")
    
    for i=1:Motor_num
        if i==Motor_num
            fprintf('[ID:%03d] GoalCurrent:%03d\n', DXL_IDs(i), Dxl_Passive_Current(1,i));
        else
            fprintf('[ID:%03d] GoalCurrent:%03d\t', DXL_IDs(i), Dxl_Passive_Current(1,i));
        end
    end
...
%     end
    ...
    for i=1:Motor_num
        if i==Motor_num
            fprintf('[ID:%03d] D_TL:%.1f\n', DXL_IDs(i), D_TL(end,i));
        else
            fprintf('[ID:%03d] D_TL:%.1f\t', DXL_IDs(i), D_TL(end,i));
        end
    end   
    pause(0.1);
    SM_Body(:,:,:,:,index)=SM_ele(:,:,[1,7,13],3);
end
%

pause(0.1);
save('Motion_Data.mat','SM_Body')
%%
